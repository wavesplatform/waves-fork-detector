...
func TestPeerLoaderSimpleOneIDsBatch(t *testing.T) {
	...
	assert.Equal(t, stageIdle, pl.sm.MustState())
	err := pl.start()
	require.NoError(t, err)
	assert.Equal(t, stageWaitingForIDs, pl.sm.MustState())
	err = pl.processIDs([]proto.BlockID{bl1.BlockID(), bl2.BlockID(), bl3.BlockID()})
	require.NoError(t, err)
	assert.Equal(t, stageWaitingForBlocks, pl.sm.MustState())
...
func TestTimeoutOnWaitingForIDs(t *testing.T) {
	...
	assert.Equal(t, stageDone, pl.sm.MustState())
